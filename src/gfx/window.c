...
static void on_char_type(GLFWwindow *glfw_window, unsigned int keycode)
{
	...
	event_publish(window->event_bus, EVENT_TYPE_CHAR_TYPE, (event_t *)&event);
}
...
static void on_mouse_move(GLFWwindow *glfw_window, double x, double y)
{
	...
	event_publish(window->event_bus, EVENT_TYPE_MOUSE_MOVE, (event_t *)&event);
...
static void on_mouse_scroll(GLFWwindow *glfw_window, double x, double y)
{
	...
	event_publish(window->event_bus, EVENT_TYPE_MOUSE_SCROLL, (event_t *)&event);
}
...
static void on_window_close(GLFWwindow *glfw_window)
{
	...
	event_publish(window->event_bus, EVENT_TYPE_WINDOW_CLOSE, (event_t *)&event);
}
...
static void on_window_resize(GLFWwindow *glfw_window, int width, int height)
{
	...
	event_publish(window->event_bus, EVENT_TYPE_WINDOW_RESIZE, (event_t *)&event);
}
...
void window_init(const window_desc_t *desc, window_t **window)
{
	...
	glfwSetCharCallback(result->glfw_window, on_char_type);
	glfwSetKeyCallback(result->glfw_window, on_key);
	glfwSetCursorPosCallback(result->glfw_window, on_mouse_move);
	glfwSetScrollCallback(result->glfw_window, on_mouse_scroll);
	glfwSetMouseButtonCallback(result->glfw_window, on_mouse_button);
	glfwSetWindowCloseCallback(result->glfw_window, on_window_close);
	glfwSetWindowSizeCallback(result->glfw_window, on_window_resize);
